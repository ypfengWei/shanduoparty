...
public class GetAccessTokenUtils {
	...
	public static UserWechat getUserWechat(String appid, String secret, String code) {
		CloseableHttpClient httpClient = HttpClients.createDefault();
		HttpGet methods = new HttpGet("https://api.weixin.qq.com/sns/oauth2/access_token?appid=APPID&secret=SECRET&code=CODE&grant_type=authorization_code".replace("APPID", appid).replace("SECRET", secret).replace("CODE", code));
		...
		HttpGet method = new HttpGet("https://api.weixin.qq.com/sns/userinfo?access_token=ACCESS_TOKEN&openid=OPENID".replace("access_token", accessToken).replace("openid", openid));
        CloseableHttpResponse response = null;
        Map<String,Object> resultMap = new HashMap<>();
        try {
            response = httpClient.execute(method);
